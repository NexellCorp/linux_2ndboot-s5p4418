...
void enterSelfRefresh(void)
{
	...
	unsigned short MR16 = 0x00 << 0;
	unsigned short MR17 = 0x00 << 0;
	...
	SetIO32(&pReg_Drex->MEMCONTROL,
			(0x1 << 0)); // clk_stop_en[0] : Dynamic Clock Control   :: 1'b0  - Always running	     
#endif	     
	DMC_Delay(1000 * 3);
}
...
void enterPowerDownMode(void)
{
	...
	SendDirectCommand(SDRAM_CMD_CKEL, 0, (SDRAM_MODE_REG)CNULL, CNULL);
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	SendDirectCommand(SDRAM_CMD_CKEL, 1, (SDRAM_MODE_REG)CNULL, CNULL);
#endif
#else
	if (pSBI->DII.ChipNum > 1)
		SendDirectCommand(SDRAM_CMD_CKEL, 1, (SDRAM_MODE_REG)CNULL,
				CNULL);
#endif
	//--------------------------------------------------------------------//	
}

void exitPowerDownMode(void)
{
#if 1
	// Step 52 Auto refresh counter enable
	SetIO32(&pReg_Drex->CONCONTROL,
...
//					printf("+ %d\r\n", code);
				...
				find_vmw = 0x0;    //- 첫 번째 PASS로부터 연속 3회 PASS 하지 못하면 연속 3회 PASS가 발생할 때까지 Searching 다시 시작하도록 "find_vmw" = "0"으로 초기화. printf("- %d\r\n", code);
			}
		} else if( (mr41 != RESP_MR41) || (mr48 != RESP_MR48) ) {
			find_vmw = 0x4;
			vwmr = code - 1;
			//            printf("-- %d\r\n", code);
			printf("\r\nMR41 = 0x%08X, MR48 = 0x%08X\r\n", mr41, mr48);
			break;
		}
...
#if (MEM_CALIBRATION_INFO)
	if (ret != CTRUE) {
		printf("\r\n CA Calibration Failed!! \r\n");	
	} else {
		printf("CA Calibration Success!! \r\n");
		printf("\r\n#### CA Calibration - Information #####\r\n");
		printf("CA Vaild Margin LEft  : %d(%d) \r\n", vwml, vwml - lock_div4);
		printf("CA Vaild Margin Center: %d(%d) \r\n", vwmc, offsetd);
		printf("CA Vaild Margin Right : %d(%d) \r\n", vwmr, vwmr - lock_div4);
		printf("###########################################\r\n");
	}
#endif
